...
Консольная игра «Выйди из тюрьмы»: убедите LLM-стражника открыть дверь.
...
Это Textual-приложение: оно работает как полноэкранный консольный интерфейс. Для браузерного режима:

```bash
make web-run SERVICE_PORT=8000
```

После запуска откройте `http://localhost:8000`. Веб-режим использует тот же Textual app и игровое ядро.
